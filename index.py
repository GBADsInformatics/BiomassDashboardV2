...
def update_graph(country, species, start, end, data, plot):
    ...
    # Prevent update when switching between select multiple (makes the graph look messy)
...
def render_content(choice, dataset, country, tab):
    if tab == 'tab-0':
        ...
        querystring = f"dashboard='biomass' AND tablename LIKE '{choice} {dataset} {country}%'"
